...
void Position::set(const string& fenStr, bool isChess960, unsigned currVariant, Thread* th) {
#else
...
#ifdef KOTH
  if (currVariant == VariantKotH)
  {
      PieceValue[MG][KNIGHT] = KnightValueKothMg;
      PieceValue[EG][KNIGHT] = KnightValueKothEg;
      PieceValue[MG][BISHOP] = BishopValueKothMg;
      PieceValue[EG][BISHOP] = BishopValueKothEg;
  }
  else
  {
...
#ifdef THREECHECK
  // 5. Checks given counter for Three-Check positions
...
void Position::variant_has_changed(int currVariant) {

  variant = currVariant;

#ifdef KOTH
  if (variant == VariantKotH)
  {
      PieceValue[MG][KNIGHT] = KnightValueKothMg;
      PieceValue[EG][KNIGHT] = KnightValueKothEg;
      PieceValue[MG][BISHOP] = BishopValueKothMg;
  ...
  // re-compute the material values
  st->nonPawnMaterial[WHITE] = st->nonPawnMaterial[BLACK] = VALUE_ZERO;

  for (Color c = WHITE; c <= BLACK; ++c)
      for (PieceType pt = KNIGHT; pt <= QUEEN; ++pt)
          st->nonPawnMaterial[c] += pieceCount[c][pt] * PieceValue[MG][pt];
}
...
#ifdef KOTH
  assert(!(is_koth() && is_koth_win()));
  assert(!(is_koth() && is_koth_loss()));
#endif
...
// XXX Consider Threecheck and KOTH
...
#if defined(THREECHECK) && defined(KOTH)
  set(f, is_chess960(), variant, this_thread());
#else
...
bool Position::pos_is_ok(int* step) const {
...
#ifdef THREECHECK
  const bool testChecksGiven     = all || false;
#endif
#ifdef KOTH
  const bool testKotHLoss        = all || false;
#endif
  ...
  if (step && ++*step, testState)
  {
      ...
      set_state(&si);
#ifdef THREECHECK
      // we have to skip the key, we don't know checkGiven at this place
      if (   (!is_three_check() && st->key != si.key)
#else
      if (   st->key != si.key
#endif
...
#ifdef KOTH
  if (step && ++*step, testKotHLoss)
  {
      if (is_koth_win() && is_koth_loss())
          return false;
  }
#endif
