...
namespace {
  ...
  const Score RookOnSemiOpenFile[5] = {
    S(19, 10),
  ...
  const Score KOTHDistanceBonus[4] = {
		...
		S(5*PawnValueMg, 5*PawnValueEg),
		S(2*PawnValueMg, 2*PawnValueEg),
		S(1*PawnValueMg, 1*PawnValueEg)
	};
  const Score KOTHDistancePenalty[8] = {
		S(0, 0),
		S(0, 0),
		S((1*PawnValueMg)/2, 1*PawnValueEg),
		S((2*PawnValueMg)/2, 2*PawnValueEg),
		S((3*PawnValueMg)/2, 3*PawnValueEg),
		S((4*PawnValueMg)/2, 4*PawnValueEg),
		S((5*PawnValueMg)/2, 5*PawnValueEg),
		S((6*PawnValueMg)/2, 6*PawnValueEg),
	};
  const Score KOTHDistancePenaltyOpening[8] = {
		S(0, 0),
		S(0, 0),
		S(0, 0),
    S((1*PawnValueMg)/8, 0),
    ...
    while ((s = *pl++) != SQ_NONE)
    {
        ...
        if (Pt == QUEEN)
		{
            ...
            if (   relative_rank(Us, s) == RANK_7
                ...
                score += QueenOn7th[pos.checks_index()];
		}
#endif
        ...
        if (Pt == ROOK)
        {
            ...
            if (   relative_rank(Us, s) == RANK_7
                && relative_rank(Us, pos.king_square(Them)) == RANK_8)
                score += RookOn7th[pos.checks_index()];
#endif
            ...
            if (ei.pi->semiopen_file(Us, file_of(s)))
                ...
                score += ei.pi->semiopen_file(Them, file_of(s))
                       ? RookOnOpenFile[pos.checks_index()]
                       : RookOnSemiOpenFile[pos.checks_index()];
#else
  ...
  Score evaluate_king(const Position& pos, const EvalInfo& ei) {
	...
	if (pos.is_koth())
	{
      ...
      // --------------------------------------------------------------------------------
      // king safety is an issue but it is not as important as in normal chess. if Queens
      // are already off the board you can safely start running with your king towards
      // the center. Better Qs are exchanged game is bit positional but once Qs are off
      // the board it can be highly tactical as many pawns and pieces can be sacrificed
      // to reach your king towards the center.

      score = score/2;

      int distance = pos.koth_distance(Us);

      if (pos.count<QUEEN>(Them) == 0 || ei.mi->game_phase() > PHASE_MIDGAME)
          score += KOTHDistanceBonus[distance];
      else if (pos.is_opening_phase())
          score -= KOTHDistancePenaltyOpening[distance];
      else
          score -= KOTHDistancePenalty[distance];
	}
#endif
    ...
    if (ei.kingAttackersCount[Them])
    {
        ...
        if (b)
        {
            ...
            if (b)
#ifdef THREECHECK
                attackUnits +=  RookContactCheck[pos.checks_index()]
                              * popcount<Max15>(b)
                              * (Them == pos.side_to_move() ? 2 : 1);
#else
  ...
  Value do_evaluate(const Position& pos) {
    ...
    if (pos.is_three_check())
    {
        assert(pos.checks_given() <= 2);
        assert(pos.checks_taken() <= 2);

        score += ChecksGivenBonus[pos.checks_given(WHITE)];
        score -= ChecksGivenBonus[pos.checks_given(BLACK)];
    }
#endif

#ifdef KOTH
    assert(!(pos.is_koth() && pos.is_koth_win()));
    assert(!(pos.is_koth() && pos.is_koth_loss()));
#endif
...
#if defined(THREECHECK) || defined(KOTH)
    struct Weight& kingSafetyWeight = Weights[KingSafety];

# ifdef THREECHECK
    if (std::string(Options["UCI_Variant"]) == "Three-Check")
    {
        kingSafetyWeight.mg = 600;
        kingSafetyWeight.eg = 300;
    }
	 else
# endif
# ifdef KOTH
    ...
    {
        kingSafetyWeight.mg = 159;
        kingSafetyWeight.eg = 0;
    }
	 else
# endif
	 {
        kingSafetyWeight.mg = 318;
        kingSafetyWeight.eg = 0;
	 }
#endif
