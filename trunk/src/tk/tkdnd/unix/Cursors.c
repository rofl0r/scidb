...
static Cursor noDropCursor	= None;
static Cursor moveCursor	= None;
static Cursor copyCursor	= None;
static Cursor linkCursor	= None;
static Cursor askCursor		= None;


void
TkDND_InitialiseCursors(Tcl_Interp* interp)
{
	XColor black, white;
	...
	if (!noDropCursor)
	{
		image_pixmap = XCreateBitmapFromData(
			display, RootWindow, (char *) noDropCurBits, noDropCursorWidth, noDropCursorHeight);
		mask_pixmap	= XCreateBitmapFromData(
			display, RootWindow, (char *) noDropCurMask, noDropCursorWidth, noDropCursorHeight);
		noDropCursor = XCreatePixmapCursor(
			display, image_pixmap, mask_pixmap, &black, &white, noDropCursorX, noDropCursorY);
	...
	{
		image_pixmap = XCreateBitmapFromData(
			display, RootWindow, (char *) CopyCurBits, CopyCursorWidth, CopyCursorHeight);
		mask_pixmap	= XCreateBitmapFromData(
			display, RootWindow, (char *) CopyCurMask, CopyCursorWidth, CopyCursorHeight);
		copyCursor = XCreatePixmapCursor(
			display, image_pixmap, mask_pixmap, &black, &white, CopyCursorX, CopyCursorY);
		XFreePixmap(display, image_pixmap);
		XFreePixmap(display, mask_pixmap);
	}
	...
	{
		image_pixmap = XCreateBitmapFromData(
			display, RootWindow, (char *) MoveCurBits, MoveCursorWidth, MoveCursorHeight);
		mask_pixmap	= XCreateBitmapFromData(
			display, RootWindow, (char *) MoveCurMask, MoveCursorWidth, MoveCursorHeight);
		moveCursor = XCreatePixmapCursor(
			display, image_pixmap, mask_pixmap, &black, &white, MoveCursorX, MoveCursorY);
		XFreePixmap(display, image_pixmap);
		XFreePixmap(display, mask_pixmap);
	}
	...
	{
		image_pixmap = XCreateBitmapFromData(
			display, RootWindow, (char *) LinkCurBits, LinkCursorWidth, LinkCursorHeight);
		mask_pixmap	= XCreateBitmapFromData(
			display, RootWindow, (char *) LinkCurMask, LinkCursorWidth, LinkCursorHeight);
		linkCursor = XCreatePixmapCursor(
			display, image_pixmap, mask_pixmap, &black, &white, LinkCursorX, LinkCursorY);
		XFreePixmap(display, image_pixmap);
		XFreePixmap(display, mask_pixmap);
	}
	...
	{
		image_pixmap = XCreateBitmapFromData(
			display, RootWindow, (char *) AskCurBits, AskCursorWidth, AskCursorHeight);
		mask_pixmap	= XCreateBitmapFromData(
			display, RootWindow, (char *) AskCurMask, AskCursorWidth, AskCursorHeight);
		askCursor = XCreatePixmapCursor(
			display, image_pixmap, mask_pixmap, &black, &white, AskCursorX, AskCursorY);
		XFreePixmap(display, image_pixmap);
		XFreePixmap(display, mask_pixmap);
	}
}
...
Tk_Cursor
TkDND_GetCursor(Tcl_Interp* interp, Tcl_Obj* name)
{
	enum DropAction
	{
		ActionCopy, ActionMove, ActionLink, ActionAsk, ActionPrivate, Refuse_Drop, ActionDefault
	};
	...
	status = Tcl_GetIndexFromObj(interp, name, (const char **) DropActions, "dropactions", 0, &index);

	if (status == TCL_OK)
	{
		switch ((enum DropAction) index)
		{
			case ActionDefault:	/* fallthru */
	...
	if ((cursor = Tk_AllocCursorFromObj(interp, Tk_MainWindow(interp), name)) == None)
	{
