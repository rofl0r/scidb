    ...
    TkTextIndex *indexPtr)
{
    ...
    if (linePtr == indexPtr->priv.linePtr && byteIndex == indexPtr->priv.byteIndex) {
	return true;
    }
    ...
    int *offset)		/* Get offset in segment, can be NULL. */
{
	...
	if (myOffset == 0) {
	    ((TkTextIndex *) indexPtr)->priv.segPtr = segPtr; /* mutable due to concept */
	}
	if (offset) {
	    *offset = myOffset;
	}
    ...
    const TkTextIndex *indexPtr2)	/* Pointer to index. */
{
    ...
    if (indexPtr1->priv.segPtr && indexPtr1->priv.segPtr == indexPtr2->priv.segPtr) {
	TkTextSegment *segPtr1 = indexPtr1->priv.segPtr;
	TkTextSegment *segPtr2 = indexPtr2->priv.segPtr;
...
TkTextIndexCompare(
    ...
    const TkTextIndex *indexPtr2)	/* Pointer to index. */
{
    const TkTextSection *sectionPtr1, *sectionPtr2;
    const TkTextSegment *segPtr1, *segPtr2;
    ...
    if (!indexPtr1->priv.segPtr || !indexPtr2->priv.segPtr) {
	return TkTextIndexGetByteIndex(indexPtr1) - TkTextIndexGetByteIndex(indexPtr2);
    }
    ...
    int byteOffset)		/* Add this offset. */
{
    ...
    if ((indexPtr->priv.byteIndex += byteOffset) >= FindEndByteIndex(indexPtr) + 1) {
	assert(indexPtr->priv.linePtr->nextPtr);
    ...
    TkTextIndex *indexPtr)	/* Structure to fill in. */
{
	...
	if (textPtr->startMarker != textPtr->sharedTextPtr->startMarker
	    ...
	    if (byteIndex <= TkTextIndexGetByteIndex(indexPtr)) {
		return indexPtr;
	    }
	    byteIndex = FindStartByteIndex(indexPtr);
	}
	if (textPtr->endMarker != textPtr->sharedTextPtr->endMarker
	    ...
	    if (TkTextIndexGetByteIndex(indexPtr) <= byteIndex) {
		return indexPtr;
	    }
	    byteIndex = FindEndByteIndex(indexPtr);
	}
    }
	...
	indexPtr->priv.byteIndex += p - start;
    }
    ...
    TkTextIndex *indexPtr)	/* Index structure to fill in. */
{
    ...
    /*
     *------------------------------------------------
     * Stage 1: start again by parsing the base index.
     *------------------------------------------------
     */
    				 ...
    				 * characters. */
{
    ...
    if (indexPtr->priv.segPtr && indexPtr->priv.segPtr->typePtr != &tkTextCharType) {
    ...
    TkTextIndex *indexPtr)	/* Index to update as specified in string. */
{
	...
	if (modifier == TKINDEX_DISPLAY) {
	     ...
	     * “+N any chars”. For backward compatibility we will still support "any", but
	     * in a future version the support of this superfluous keyword shall be eliminated.
	     */
    ...
    const TkTextIndex *indexPtr)/* Index describing location of character. */
{
    ...
    Tcl_UtfToUniChar(segPtr->body.chars + byteOffset, &ch);
    return ch;
}

/*
