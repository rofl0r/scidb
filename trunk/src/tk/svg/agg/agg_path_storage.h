...
namespace agg
{
    ...
    void vertex_block_storage<T,S,P>::allocate_block(unsigned nb)
    {
        if(nb >= m_max_blocks)
        {
            ...
            unsigned char** new_cmds =
                reinterpret_cast<unsigned char**>(new_coords + m_max_blocks + block_pool);
        ...
        m_cmd_blocks[nb]  =
            reinterpret_cast<unsigned char*>(m_coord_blocks[nb] + block_size * 2);
    ...
    //------------------------------------------------------------------------
    template<class VC>
    void path_base<VC>::rel_to_abs(double* x, double* y) const
    {
        ...
        if(orientation != path_flags_none)
        {
            while(start < m_vertices.total_vertices())
            {
                start = arrange_polygon_orientation(start, orientation);
					 if(start >= m_vertices.total_vertices())	// NOTE: fix by GC
					 {
						 break;
					 }
                else if(is_stop(m_vertices.command(start)))
                {
                    ++start;
                    break;
                }
