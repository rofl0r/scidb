...
namespace agg
{
    ...
    public:
        ...
        {
            static_cast<value_type*>(p)[order_type::R] = c.r;
            static_cast<value_type*>(p)[order_type::G] = c.g;
            static_cast<value_type*>(p)[order_type::B] = c.b;
            static_cast<value_type*>(p)[order_type::A] = c.a;
        }

        //--------------------------------------------------------------------
        AGG_INLINE color_type pixel(int x, int y) const
        {
            const value_type* p = static_cast<const value_type*>(m_rbuf->row_ptr(y));
            if(p)
            {
                p += x << 2;
        ...
        AGG_INLINE void blend_pixel(int x, int y, const color_type& c, int8u cover)
        {
            cob_type::copy_or_blend_pix(
                static_cast<value_type*>(m_rbuf->row_ptr(x, y, 1)) + (x << 2), 
                c.r, c.g, c.b, c.a, 
                cover);
        }
                                   ...
                                   const color_type& c)
        {
            ...
            reinterpret_cast<value_type*>(&v)[order_type::B] = c.b;
            reinterpret_cast<value_type*>(&v)[order_type::A] = c.a;
            do
            {
                *reinterpret_cast<pixel_type*>(p) = v;
                p += 4;
            }
            while(--len);
        }
        ...
        template<class Function> void for_each_pixel(Function f)
        {
            ...
            for(y = 0; y < height(); ++y)
            {
                ...
                if(r.ptr)
                {
                    ...
                    value_type* p = 
                        static_cast<value_type*>(m_rbuf->row_ptr(r.x1, y, len)) + (r.x1 << 2);
                    do
                    {
                        f(p);
                        p += 4;
                              ...
                              int8u cover)
        {
            ...
            const src_value_type* psrc = static_cast<src_value_type*>(from.row_ptr(ysrc));
            if(psrc)
            {
                value_type* pdst = 
    ...
    public:
        ...
        {
            const value_type* p = static_cast<value_type*>(m_rbuf->row_ptr(y)) + (x << 2);
            return color_type(p[order_type::R], 
                              p[order_type::G], 
                              p[order_type::B], 
                              p[order_type::A]);
        ...
        {
            typedef typename SrcPixelFormatRenderer::order_type src_order;
            const value_type* psrc = static_cast<const value_type*>(from.row_ptr(ysrc));
            if(psrc)
            {
                psrc += xsrc << 2;
                value_type* pdst = 
