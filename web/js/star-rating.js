...
/*
 * Some modifications and fixes by Gregor Cramer, 2015.
 */
...
(function ($) {
    ...
    Rating.prototype = {
        ...
        listen: function () {
            ...
            self.$rating.on("click", function (e) {
                ...
                if (self.normalColor !== 'undefined') {
                  self.$stars.css('color', self.normalColor);
                }
            ...
            self.$rating.on("mouseenter", function (e) {
                if (!self.hoverEnabled || self.inactive || self.starClicked) {
                    return;
                }
                if (self.hoverColor !== 'undefined') {
                  self.normalColor = self.$stars.css('color');
                  self.$stars.css('color', self.hoverColor);
                }
            });
            ...
            self.$clear.on("mousemove", function (e) {
                ...
                var val = self.clearValue, width = self.getWidthFromValue(val);
                var out = {caption: caption, width: width, val: val};
                self.toggleHover(out);
        ...
        initSlider: function (options) {
            ...
            if (self.initialValue == 'undefined' || self.initialValue == 0) {
        ...
        init: function (options) {
            ...
            var defaultSpace = (self.glyphicon) ? '\u200a' : '';
            self.space = isEmpty(options.space) ? defaultSpace : options.space;
            ...
            if (typeof self.$rating == 'undefined' && typeof self.$container == 'undefined') {
                ...
                var color = self.$rating.css('color');
                self.$rating.addClass('rating-stars-hover');
                if (self.$rating.css('color') !== color) {
                   self.hoverColor = self.$rating.css('color');
                }
                self.$rating.removeClass('rating-stars-hover');
            }
            if (typeof self.$element.attr('showClear') !== 'undefined') {
               self.showClear = self.$element.attr('showClear') == 'true';
            }
            if (typeof self.$element.attr('showCaption') !== 'undefined') {
               self.showClear = self.$element.attr('showCaption') == 'true';
            }
            if (typeof self.$element.attr('value') !== 'undefined') {
               self.initialValue = parseFloat(self.$element.attr('value'));
               self.$element.val(self.initialValue);
            } else if (self.initialValue !== 'undefined') {
               self.$element.val(self.initialValue);
            }
        ...
        getWidthFromValue: function (val) {
            ...
            return Math.ceil(((val - min) * 100.0) / (max - min));
        },
        ...
        calculate: function (pos) {
            var self = this;
            var defaultVal = isEmpty(self.$element.val()) ? self.min : self.$element.val();
            var val = arguments.length ? self.getValueFromPosition(pos) : defaultVal;
            var caption = self.fetchCaption(val), width = self.getWidthFromValue(val);
            if (self.rtl) {
                width = 100 - width;
            }
            width += '%';
    ...
    $.fn.rating.defaults = {
        ...
        clearCaptionClass: 'label label-0',
        clearValue: null,
        captionElement: null,
        clearElement: null,
        containerClass: null,
